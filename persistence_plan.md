...
Add a small NVS-backed settings layer so `RustwoodConfig` is loaded on boot and can be written back from the web UI without changing the current live-apply flow. The form should keep its existing Apply behavior, and the new Save button should submit the same form values, update the shared config mutex, and then persist those values to flash so they survive power cycles.

**Steps**
1. Add the persistence dependency and data model support in the smallest possible surface area, reusing `RustwoodConfig` as the single source of truth for both live state and stored state.
...
3. Update startup initialization in `src/bin/main.rs` to load persisted config before creating the shared mutex, falling back to `RustwoodConfig::default()` when nothing is saved or decode fails.
...
**Relevant files**
- `/home/bidscc/git/rustwood/Cargo.toml` — add the NVS/persistence dependency and any serialization support needed for `RustwoodConfig`.
- `/home/bidscc/git/rustwood/src/lib.rs` — keep `RustwoodConfig` as the shared config type and add any helpers needed for storing/loading it.
...
- Persistence scope is limited to `RustwoodConfig`; no other runtime state should be written to flash.
